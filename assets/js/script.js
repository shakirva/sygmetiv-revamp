...
const boxes = [
  {
    ...
    image: "./assets/images/erp-01.jpg"
  },
  {
    ...
    image: "./assets/images/Business-App.jpg"
  },
  {
...
function toggleDisplayClass() {
  ...
  if (viewportWidth > tabViewportWidth) {
    ...
    if (serviceWrapper) serviceWrapper.classList.add("display");
    if (dServiceWrapper) dServiceWrapper.classList.remove("display"); // Make sure it's removed from the other element
    if (footer) footer.classList.add("display");
    if (Dfooter) Dfooter.classList.remove("display");
    if (heroBtn[1]) heroBtn[1].classList.add("display");
    if (heroBtn[0]) heroBtn[0].classList.remove("display");
    // aboutImg.style.display = "block";
  } else {
    // If viewport is smaller than or equal to tab, add "display" class to d-service-wrapper
    if (dServiceWrapper) dServiceWrapper.classList.add("display");
    if (serviceWrapper) serviceWrapper.classList.remove("display"); // Remove it from the other element
    if (Dfooter) Dfooter.classList.add("display");
    if (footer) footer.classList.remove("display");
    if (heroBtn[1]) heroBtn[0].classList.add("display");
    if (heroBtn[0]) heroBtn[1].classList.remove("display");
